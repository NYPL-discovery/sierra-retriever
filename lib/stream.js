...
exports.streamPoster = (records, streamName) => {
  return new Promise((resolve, reject) => {
    streamsClient.write(streamName, records)
      .then((resp) => {
        if(resp.FailedRecordCount > 0)
          reject({sent: records.length, received: records.length - resp.FailedRecordCount})
        log.info('Finished writing to stream. Records - ' + resp.Records.length)
        resolve({sent: records.length, receieved: resp.Records.length})
      })
      .catch((error) => {
